...
<div class="container mx-auto p-6">
    ...
    <div class="mb-8">
        ...
        @forelse ($customers as $customer)
            <div class="bg-white shadow-md rounded-lg p-6 mb-4 flex justify-between items-center">
                ...
                <div>
                    <form action="{{ route('loan_officer.manage_user', $customer->id) }}" method="POST">
                        @csrf
    ...
    @if(auth()->user()->role == 'loan_officer')
        <div class="mb-8">
            ...
            @php
                $loanApplications = \App\Models\Loan::where('status', 'pending')->where('loan_officer_id', auth()->user()->id)->get();
            @endphp
            @forelse ($loanApplications as $loan)
                <div class="bg-white shadow-md rounded-lg p-6 mb-4 flex justify-between items-center">
                    ...
                    <div>
                         <a href="{{ route('loan_officer.show', $loan->id) }}" class="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                            View Details
                        </a>
                    </div>
                </div>
